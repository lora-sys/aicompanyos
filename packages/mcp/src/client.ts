...
export class MCPClientAdapter {
  ...
  private _onServerConnected: EventEmitter<MCPServerInfo>;

  /** Server 已断开事件 */
  readonly onServerDisconnected: Event<string>;
  private _onServerDisconnected: EventEmitter<string>;

  /** Server 错误事件 */
  readonly onServerError: Event<{ name: string; error: Error }>;
  private _onServerError: EventEmitter<{ name: string; error: Error }>;
  ...
  async connect(serverConfig: MCPServerConfig): Promise<MCPServerInfo> {
    ...
    try {
      ...
      const client = new Client(
        { name: "aicos-mcp-client", version: "0.1.0" },
  ...
  async connectAll(
    ...
    const results = new Map<string, MCPServerInfo>();
  ...
  async callTool(request: MCPToolCallRequest): Promise<MCPToolCallResult> {
    ...
    if (conn.info.status !== MCPConnectionStatus.CONNECTED) {
      throw new Error(
        `MCP Server "${serverName}" 当前状态为 ${conn.info.status}，无法调用工具`
      );
    }
    ...
    if (!toolExists) {
      throw new Error(
        `工具 "${toolName}" 在 MCP Server "${serverName}" 上不存在`
      );
    }
