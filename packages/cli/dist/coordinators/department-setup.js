...
import { GenericAgent, WORKER_ROLES, } from "@aicos/loop-engine";
import { ContentProductionDepartment, OutputPipeline, } from "@aicos/content-production";
...
export class DepartmentSetup {
        ...
        try {
            ...
            if (deptConfig.teamManager) {
                try {
                    ...
                    this.deps.addLog("info", "team", `动态团队组建完成: ${team.workers.length} 人 [${workerTypes}] (规则: ${team.matchedRuleId})`);
                    ...
                    if (deptConfig.teamManager.createWorkerFactoriesWithDeps) {
                        ...
                        for (const [agentType, factory] of Object.entries(factories)) {
                    ...
                    this.deps.addLog("warn", "team", `动态团队组建失败（降级到默认双核）: ${teamErr instanceof Error ? teamErr.message : teamErr}`);
                }
            }
            const typeLabel = this.contentDept.getAvailableTypes().find((t) => t.type === resolvedType)?.label ?? resolvedType;
            this.deps.addLog("info", "department", `已切换到内容产出部 → ${typeLabel} (${resolvedType})`);
            console.log(`\n✅ 已切换到: ${typeLabel}\n   Writer Prompt 已注入 | GoalTemplates 已加载 | OutputPipeline 已配置\n`);
        }
        catch (e) {
            this.deps.addLog("error", "department", `部门配置加载失败: ${e instanceof Error ? e.message : e}`);
        }
    }
    /**
     ...
     */
    registerDynamicWorker(agentType, role, contentType) {
        if (agentType === "researcher") {
            this.deps.loopHarness.registerAgent(agentType, () => new ResearcherAgent(this.deps.toolRegistry, this.deps.llmProvider));
        ...
        if (agentType === "reviewer") {
            this.deps.loopHarness.registerAgent(agentType, () => new ReviewerAgent(this.deps.llmProvider));
        ...
        this.deps.loopHarness.registerAgent(agentType, () => new GenericAgent({ systemPrompt, llmProvider: this.deps.llmProvider }));
        this.deps.addLog("info", "team", `已注册 Worker: ${agentType} (${role}) → GenericAgent`);
    }
    /** 根据 agentType 构建 GenericAgent 的系统提示（可扩展为部门专属 Prompt） */
    buildGenericAgentPrompt(agentType, contentType) {
...
//# sourceMappingURL=department-setup.js.map