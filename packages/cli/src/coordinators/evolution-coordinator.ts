...
export interface EvolutionCoordinatorDeps {
  ...
  getTaskId: () => string;
  getTaskInput: () => string;
  getLoopContext: () => { consensusRound?: number; retryCount?: number } | null;
  ...
  getCollectedToolCalls: () => Array<{ toolName: string; success: boolean; duration?: number }>;
...
export class EvolutionCoordinator {
  private deps: EvolutionCoordinatorDeps;
  ...
  async run(params: {
    ...
    lastGuardSummary?: { totalGoals: number; verifiedGoals: number };
  }): Promise<void> {
  ...
  private buildEvidenceReader(): IEvidenceReader {
  ...
  private buildTaskMetrics(): TaskMetrics {
