...
import type { Artifact } from "@aicos/loop-engine";
...
export interface ExecutionCoordinatorDeps {
  ...
  getCollectedVerifications: () => Array<{ goalId: string; verified: boolean; evidence?: string }>;
}
...
export interface ExecutionCoordinatorResult {
  ...
  guardSummary?: { totalGoals: number; verifiedGoals: number };
}
...
export class ExecutionCoordinator {
  ...
  async run(plan: import("@aicos/loop-engine").ExecutionPlan): Promise<ExecutionCoordinatorResult> {
    ...
    try {
      const historyReader = new HistoryReader(
        () => memoryManager.evolution.getSelfMD(),
        () => memoryManager.evolution.getUserMD(),
      );
      ...
      if (historyResult.promptPrefix) {
        ...
        onLog("info", "memory",
          `历史上下文已注入: ${historyResult.stats.experienceCount} 条经验, ` +
  ...
  private extractSummaries(result: HarnessExecutionResult): {
    criticSummary?: CriticSummary;
    guardSummary?: { totalGoals: number; verifiedGoals: number };
  } {
    const writerSteps = result.stepResults.filter((s) => s.iterations.length > 0);
    if (writerSteps.length === 0) return {};
    ...
    const criticSummary: CriticSummary | undefined = critic
      ? {
  ...
  private async persistOutputsToDisk(result: HarnessExecutionResult): Promise<void> {
    ...
    for (const [stepId, output] of Object.entries(result.finalOutputs)) {
      try {
        ...
        this.deps.onLog("info", "execute",
          `✅ 产物已写入磁盘: ${artifact.name} (${artifact.sizeBytes} bytes)`
        );
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        this.deps.onLog("warn", "execute",
          `⚠️ Step "${stepId}" 产物写入失败（非致命）: ${msg}`
        );
      }
    }
    ...
    if (result.processedOutput) {
      try {
        ...
        this.deps.onLog("info", "execute",
          `✅ 后处理产物已写入磁盘: ${artifact.name} (${artifact.sizeBytes} bytes)`
        );
        ...
        if (processed.rawContent && processed.format !== "markdown") {
          ...
          if (!existingRaw) {
            ...
            this.deps.onLog("info", "execute",
              `✅ 原始 Markdown 已写入磁盘: ${rawArtifact.name} (${rawArtifact.sizeBytes} bytes)`
            );
          }
        }
      } catch (e) {
  ...
  private extractContentFromOutput(output: unknown): string | null {
