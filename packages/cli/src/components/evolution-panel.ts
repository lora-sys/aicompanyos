 ...
 * 进化阶段标签映射
 */
const PHASE_LABELS: Record<EvolutionPanelData["phase"], string> = {
  analyzing: "分析现有成果...",
  generating: "生成进化方案...",
  applying: "应用进化变更...",
  complete: "进化完成",
 ...
 */
export function formatEvolutionString(data: EvolutionPanelData): string {
  const lines: string[] = [];
 ...
 * 进化动画帧（用于流式展示）
 */
export function getEvolutionAnimationFrame(
  frameIndex: number
): { phase: EvolutionPanelData["phase"]; progress: number } {
