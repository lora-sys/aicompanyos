...
import type { LoopVisualizationData, LoopStepProgress } from "../types.js";

/**
 * Loop 流程图定义
 * 定义各步骤的标签和顺序
 */
const LOOP_STEPS: Array<{ state: LoopState; label: string }> = [
 ...
 */
export function formatLoopASCII(data: LoopVisualizationData): string {
  const lines: string[] = [];
  ...
  lines.push(
    `│  ${formatStep(data.steps[0])} → ${formatStep(data.steps[1])} → ${formatStep(data.steps[2])}     │`
  );
  ...
  lines.push(
    `│        ${formatStep(data.steps[3])} → ${formatStep(data.steps[4])}     │`
  );
  ...
  lines.push(
    `│     [REPLAN]   ${formatStep(data.steps[5])}        │`
  );
 ...
 * 格式化单个步骤的状态标记
 */
function formatStep(step: LoopStepProgress): string {
  switch (step.status) {
    case "done":
