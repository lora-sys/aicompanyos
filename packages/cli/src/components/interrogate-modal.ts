...
import type {
  ...
  InterrogateEngine,
} from "@aicos/loop-engine/interrogate";
import type {
  ModalRenderResult,
  ModalAction,
  QuestionCardData,
...
export class InterrogateModal {
  private session: InterrogationSession;
  private engine: InterrogateEngine;
  private currentIndex: number = 0;
  private inputValue: string = "";
  /** 是否处于摘要确认模式 */
  private summaryMode: boolean = false;
  /** 摘要模式下的导航索引 */
  private summaryNavIndex: number = 0;
   ...
   * 渲染问题卡片
   */
  private renderQuestion(): ModalRenderResult {
   ...
   * 渲染摘要卡片
   */
  private renderSummary(): ModalRenderResult {
    return {
      type: "summary",
   ...
   * 已完成 ● / 当前 ◉ / 待完成 ○
   */
  private buildProgressDots(current: number, total: string | number): string {
    const dots: string[] = [];
   ...
   * 展示之前已回答问题的维度和简要内容
   */
  private buildCollectedInfo(): string[] {
    const info: string[] = [];
   ...
   * 在摘要模式下导航到指定 Q&A 对
   */
  private navigateSummary(direction: number): ModalAction {
    const totalQAPairs = this.session.questions.length;
    let newIndex = this.summaryNavIndex + direction;

    // 边界检查循环
    if (newIndex < 0) newIndex = totalQAPairs - 1;
    if (newIndex >= totalQAPairs) newIndex = 0;
