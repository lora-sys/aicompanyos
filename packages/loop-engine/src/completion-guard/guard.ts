...
interface EvidenceChainLike {
  verifications?: {
    record(params: {
      ...
      passed: boolean;
      durationMs: number;
      evidenceSummary: { methodType: string; passed: boolean; keyOutput: string };
      round: number;
      taskId: string;
    }): unknown;
  };
...
export class CompletionGuard {
  ...
  private evidenceChain: EvidenceChainLike | undefined;
  private effortSpent = 0;
  private roundCount = 0;
  private taskId: string;

  /** v0.3.1+: 最新质量分数（由 LoopModule 每轮迭代后注入） */
  private currentQualityScore: number | undefined;

  constructor(
    goals: AcceptanceGoal[],
    config?: Partial<CompletionGuardConfig>,
    evidenceChain?: EvidenceChainLike,
    ...
    // 初始化所有目标为 pending
    this.goals = new Map(
      goals.map((g) => [g.id, { state: "pending" as const, goal: g }])
    );
  }
   ...
   * 6. 计算 stopCondition
   */
  async check(currentOutput?: unknown, context?: Partial<VerificationContext>): Promise<CompletionCheckResult> {
    this.roundCount++;
    ...
    // 3. 执行验证
    const previousStates = new Map<string, GoalStatus["state"]>();
    for (const [id, status] of this.goals) {
      previousStates.set(id, status.state);
    }

    const evidences: EvidenceRecord[] = [];
    const checkedGoals: CompletionCheckResult["checkedGoals"] = [];
    ...
    console.log(
      `[CompletionGuard] Round ${this.roundCount}: ` +
  ...
  getProgress(): CompletionProgress {
    ...
    for (const [, gs] of this.goals) {
      switch (gs.state) {
        ...
        case "failed": failed++; remainingEffort += this.goalWeight(gs.goal); break;
        case "pending": pending++; remainingEffort += this.goalWeight(gs.goal); break;
   ...
   */
  private async verifyGoal(goal: AcceptanceGoal, ctx: VerificationContext): Promise<GoalStatus> {
    // 标记为验证中
    const verifyingStatus: GoalStatus = {
      state: "verifying",
      goal,
      startedAt: new Date(),
    };
    ...
    for (const method of goal.verifyBy) {
      try {
        ...
        const recordWithGoalId: EvidenceRecord = { ...evidence, goalId: goal.id };
   ...
   * 4. 否则             → null (继续)
   */
  private determineStopCondition(): StopCondition | null {
    const verifiedGoals: Array<{ goalId: string; evidence: EvidenceRecord }> = [];
    const blockedGoals: Array<{ goalId: string; blocker: BlockerReason }> = [];
    const pendingGoals: string[] = [];
    const remainingGoals: MaxEffortExceededStop["remainingGoals"] = [];
  ...
  private getVerifiableGoals(): Array<{ goal: AcceptanceGoal; retryCount: number }> {
    const result: Array<{ goal: AcceptanceGoal; retryCount: number }> = [];
  ...
  private goalWeight(goal: AcceptanceGoal): number {
  ...
  /** 计算一轮验证消耗的努力值 */
  private calculateEffort(verifiedGoals: Array<{ goal: AcceptanceGoal; retryCount: number }>): number {
   ...
   * - 连续多次失败且错误信息相同
   */
  private analyzeBlocker(_goal: AcceptanceGoal, evidence: EvidenceRecord): BlockerReason | null {
  ...
  private recordEvidence(evidence: EvidenceRecord, _goalId: string): void {
    if (!this.evidenceChain?.verifications) return;
