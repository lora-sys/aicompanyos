...
export class VerificationPipeline {
  ...
  constructor(config?: { timeoutMs?: number; llmProvider?: (prompt: string) => Promise<string> }) {
  ...
  async execute(method: VerificationMethod, context: VerificationContext): Promise<EvidenceRecord> {
    ...
    const result = await Promise.race([
      ...
      new Promise<EvidenceRecord>((resolve) =>
        setTimeout(() => {
          resolve({
            goalId: "",
            method: method.type,
  ...
  async executeParallel(
    ...
    for (let i = 0; i < methods.length; i += effectiveConcurrency) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map((method) => this.execute(method, context))
      );
  ...
  /** 从验证方法中提取超时配置 */
  private extractTimeout(method: VerificationMethod): number | undefined {
