...
export class TeamComposer {
  ...
  private sortedRules: TeamCompositionRule[];

  /** 是否有兜底规则 */
  private hasFallbackRule: boolean;
  ...
  constructor(rules: TeamCompositionRule[]) {
    ...
    // 检查是否有兜底规则
    this.hasFallbackRule = rules.some(
      (r) => r.priority >= 998
    );

    if (!this.hasFallbackRule) {
      console.warn(
        "[TeamComposer] 警告：没有兜底规则（match 始终返回 true 或 priority>=999）。建议添加默认规则以避免无法组队。"
      );
    }
  }
  ...
  compose(features: TaskFeatures): TeamWorkerDef[] {
    for (const rule of this.sortedRules) {
      try {
        if (rule.match(features)) {
          console.log(
            `[TeamComposer] 命中规则 "${rule.id}" (priority=${rule.priority}): ${rule.reasoning}`
          );
          return rule.team;
        }
      } catch (e) {
    ...
    // 没有任何规则命中且没有兜底规则
    throw new Error(
      `[TeamComposer] 没有规则匹配当前任务特征（domain=${features.domain}, ` +
  ...
  private roleToAgentType(role: WorkerRole): string {
    const mapping: Record<WorkerRole, string> = {
      writer: "writer",
      critic: "critic",
