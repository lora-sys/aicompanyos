...
function createMockSelfData(overrides?: Partial<SelfMemoryData>): SelfMemoryData {
  return {
...
function createMockUserData(overrides?: Partial<UserMemoryData>): UserMemoryData {
  const baseProfile = {
    writingStyle: "专业但亲切",
...
describe("HistoryReader — 能力清单", () => {
  it("应按熟练度降序排列能力", async () => {
    const reader = new HistoryReader(async () =>
      createMockSelfData({
        capabilities: [
  ...
  it("maxCapabilities 应限制数量", async () => {
    const caps: Capability[] = Array.from({ length: 10 }, (_, i) => ({
      name: `能力${i + 1}`,
      proficiency: 80 - i * 5,
      lastUsed: "2026-06-01",
      successCount: 5,
...
describe("HistoryReader — 经验教训", () => {
  it("应包含成功和失败的经验", async () => {
    const experiences: SelfExperienceEntry[] = [
    ...
    const reader = new HistoryReader(async () =>
      createMockSelfData({ experiences })
    );
  ...
  it("maxExperiences 应限制数量", async () => {
    const experiences: SelfExperienceEntry[] = Array.from({ length: 10 }, (_, i) => ({
      entryId: `e${i}`,
      pattern: `任务${i}`,
    ...
    const reader = new HistoryReader(
      async () => createMockSelfData({ experiences }),
      undefined,
      { maxExperiences: 3 }
    );
...
describe("HistoryReader — 用户画像", () => {
  it("应包含用户画像信息", async () => {
    const reader = new HistoryReader(
      async () => createMockSelfData({ experiences: [{ entryId: "e1", pattern: "test", type: "success", lesson: "ok", timestamp: "2026-06-01", taskType: "article" }] }),
