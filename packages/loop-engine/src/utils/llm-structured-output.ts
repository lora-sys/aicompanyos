 ...
 * 此方法是纯函数，无副作用，可独立使用。
 */
export function extractJSON(raw: string): JSONExtractResult | null {
  const text = raw.trim();
  if (!text) return null;
  ...
  if (codeBlockMatch) {
    ...
    if (inner && isValidJSON(inner)) return { json: inner, source: "codeblock" };
  }
 ...
 * ```
 */
export class LLMStructuredOutput<T> {
  ...
  parse(raw: string): ParseResult<T> {
    ...
    try {
      ...
      return this.handleFailure(
        `JSON 语法错误 (${extracted.source}): ${(parseError as Error).message}`
      );
    }
    ...
    return this.handleFailure(
      `Schema 验证失败 (${extracted.source}): ${validationErrors.slice(0, 3).join("; ")}`,
      validationErrors
    );
  }
  ...
  private handleFailure(error: string, validationErrors?: string[]): ParseFailure<T> {
    ...
    switch (this.strategy) {
      ...
      case FallbackStrategy.RETURN_NULL:
        ...
        return { success: false, error, fallback: this.fallback as never, validationErrors };
...
function formatZodErrors(zodError: ZodError): string[] {
  ...
  if (issues.length === 0) return [zodError.message];

  return issues.map((issue) => {
    // Zod v4 的 issue 类型结构较复杂，使用安全访问
    const pathStr = Array.isArray(issue.path)
      ? (issue.path.length > 0 ? `.${(issue.path as string[]).join(".")}` : "")
      : "";
    const detail = issue.code === "invalid_type"
      ? `期望 ${String((issue as { expected?: string }).expected ?? "未知")}`
 ...
 * ```
 */
export function createLLMParser<T>(options: {
  schema: ZodSchema<T>;
  fallback: T;
  strategy?: FallbackStrategy;
  logPrefix?: string;
