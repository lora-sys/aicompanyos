...
}) as LLMStructuredOutput<{ score: number; reasons: string[] }>;
...
export class VerifyEngine {
    ...
    if (this.config.checkFileExistence) {
      ...
      if (fileFailures.length > 0) {
        return {
          passed: false,
          score: 0,
          reasons: fileFailures.map((c) =>
            !c.exists ? `文件不存在: ${c.path}` : `文件为空: ${c.path}`
          ),
          artifactChecks,
        };
      }
   ...
   * 文件存在性检查
   */
  private async checkFiles(artifacts: string[]): Promise<VerifyResult["artifactChecks"]> {
...
## 执行计划摘要
${input.plan?.steps?.length ? `共 ${input.plan.steps.length} 个步骤: ${input.plan.steps.map((s: { title?: string; description?: string }) => s.title || s.description || "未命名").join(", ")}` : "（无计划信息）"}
