...
export interface GradingDimension {
  ...
  examples?: Array<{
    description: string; // 描述什么样的产出
    score: number; // 应该得多少分
    reason: string; // 为什么是这个分
  }>;
}
 ...
 */
export const DEFAULT_WRITING_CRITERIA: GradingCriteria = {
 ...
 */
export function formatCriteriaForEvaluator(criteria: GradingCriteria): string {
  const lines: string[] = [
 ...
 */
export function formatCriteriaForGenerator(criteria: GradingCriteria): string {
  const lines: string[] = [
    `═══ 写作质量标准 (你需要达到的目标) ═══`,
    ``,
