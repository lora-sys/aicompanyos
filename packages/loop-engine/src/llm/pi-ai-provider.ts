...
// pi-ai 类型导入（动态导入以避免硬依赖）
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type PiAiComplete = (...args: any[]) => Promise<{
...
export class PiAILLMProvider implements LLMProvider {
  ...
  private useDirectFetch: boolean = false;
  private piAiComplete: PiAiComplete | null = null;
  ...
  private async chatWithPiAi(
    ...
    if (!textParts) {
      ...
      const thinkingParts = response.content
        ...
        .join("");
      if (thinkingParts) return thinkingParts;
   ...
   */
  static async fromEnv(): Promise<PiAILLMProvider> {
 ...
 * 支持多种 .env 位置
 */
async function loadEnvConfig(): Promise<{
  apiKey: string;
  apiBase: string;
  model: string;
}> {
