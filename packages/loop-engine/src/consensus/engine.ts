...
import type { AgentExecutor } from "../orchestrator/types.js"; // 保留用于 UI-UX reviewer（向后兼容）
import { z } from "zod";
import {
  createLLMParser,
  FallbackStrategy,
...
export class ConsensusLock {
  ...
  private writerReviewer?: IEvaluatorAgent; // 使用 IEvaluatorAgent 替代 AgentExecutor
  private uiuxReviewer?: AgentExecutor; // UI-UX Agent（可选，保留 AgentExecutor 向后兼容）
  private llmProvider?: LLMProvider;
  private criteria: GradingCriteria | undefined; // IEvaluatorAgent.evaluate() 需要的评估标准
  ...
  async reachConsensus(params: {
    ...
    let lastResult: ConsensusResult | undefined;
    ...
    if (this.writerReviewer) {
      try {
        const writerOutputObj = typeof writerOutput === "string"
          ...
          : (writerOutput as { content?: string; [key: string]: unknown });
        ...
        const gradingResult = await this.writerReviewer.evaluate(
          writerOutputObj,
          this.criteria,
          context.taskInput ?? ""
        );
        ...
        votes.push({ voter: "critic", vote: ConsensusVote.ABSTAIN, error: String(error) } as never);
      }
    ...
    if (this.config.enableUIUXVoting && this.uiuxReviewer) {
      try {
        const uiuxOutput = await this.uiuxReviewer.execute({
          ...
          tools: {} as import("../tool-registry/registry.js").ToolRegistry,
          context,
          previousOutputs: { writerOutput: { content: String(writerOutput ?? "") } },
        });
        votes.push(this.parseVote(uiuxOutput, "ui-ux"));
      } catch (error) {
        votes.push({ voter: "ui-ux", vote: ConsensusVote.ABSTAIN, error: String(error) } as never);
      }
   ...
   * 使用 LLM 进行投票（降级方案）
   */
  private async llmVote(
    writerOutput: unknown,
    voterName: string,
    systemPrompt: string
  ): Promise<ConsensusResult["votes"][number]> {
    if (!this.llmProvider) {
   ...
   */
  private parseVote(raw: unknown, voter: string): ConsensusResult["votes"][number] {
    ...
    if (!parsed) return this.fallbackParseVote(raw, voter);

    // 确保关键字段存在
    return {
      voter,
      vote: (parsed.vote as ConsensusVote) || ConsensusVote.ABSTAIN,
  ...
  private parseGradingVote(
    grading: { totalScore: number; passed: boolean; excellent: boolean },
    voter: string
  ): ConsensusResult["votes"][number] {
    return {
      voter,
      vote: grading.passed
   ...
   */
  private fallbackParseVote(raw: string, voter: string): ConsensusResult["votes"][number] {
    try {
      ...
      if (objMatch) {
        ...
        return {
          voter,
          vote: (parsed.vote as ConsensusVote) ||
   ...
   * 综合评估投票结果
   */
  private evaluateVotes(votes: ConsensusResult["votes"]): {
    passed: boolean;
    verdict: string;
    avgScore?: number;
    dominantVote?: string;
    ...
    if (scoredVotes.length > 0) {
      const avgScore =
        scoredVotes.reduce((sum, v) => sum + (v.score ?? 0), 0) / scoredVotes.length;
