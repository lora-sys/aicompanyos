...
// 所有合法的状态转换映射表
const VALID_TRANSITIONS: Map<LoopState, LoopState[]> = new Map([
...
export class LoopStateMachine {
  ...
  private readonly _context: LoopContext;
  private readonly _guards: Map<string, TransitionGuard[]> = new Map();
  private readonly _onEnterHooks: Map<LoopState, StateHook[]> = new Map();
  private readonly _onExitHooks: Map<LoopState, StateHook[]> = new Map();
  ...
  async transition(
    ...
    if (!this.canTransition(to)) {
      throw new Error(
        `非法状态转换: ${from} -> ${to}。当前状态允许的转换目标: ${this.getValidTransitions().join(", ")}`
      );
    }

    // 构建转换对象（用于守卫和事件）
    const transition: StateTransition = { from, to } as StateTransition;
    ...
    if (
      ...
      throw new Error(
        `Replan 转换失败：重试次数已达上限 (${MAX_RETRY_COUNT})，当前 retryCount=${this._context.retryCount}`
      );
    }
   ...
   * 生成转换的唯一键值（用于 Map 存储）
   */
  private transitionKey(from: LoopState, to: LoopState): string {
    return `${from}->${to}`;
  }
}
