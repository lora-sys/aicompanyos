...
export const DEFAULT_STOP_POLICY_CONFIG: StopPolicyConfig = {
  maxIterations: 5,
 ...
 */
export function evaluateStop(ctx: StopContext, config: StopPolicyConfig): StopDecision {
...
export function isSignificantImprovement(
  ...
  lastScore: number,
  improvementThreshold: number
): "new_best" | "improved" | "stagnant" {
  if (currentScore > bestScore) return "new_best";
  if (currentScore > lastScore + improvementThreshold) return "improved";
  return "stagnant";
}

export class StopPolicy {
  private config: StopPolicyConfig;
