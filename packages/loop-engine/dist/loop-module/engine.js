 ...
 */
function extractOriginalTask(input) {
...
export class LoopModule {
    ...
    async run(input) {
        ...
        while (!this.shouldStop(iterations, bestScore, lastScore, round)) {
            ...
            let evaluation;
            try {
                evaluation = await retryWithBackoff(() => this.evaluator.evaluate(output, this.criteria, 
    ...
    shouldStop(iterations, _bestScore, _lastScore, currentIteration) {
        ...
        if (stopped) {
            console.log(`[LoopModule] shouldStop() → StopPolicy 触发停止: ${lastIter.stopReason}`);
        }
        return stopped;
    }
    /** 推断当前战略方向 */
