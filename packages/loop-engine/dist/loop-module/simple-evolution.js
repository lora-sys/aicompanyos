export class SimpleEvolutionAgent {
    ...
    /** 分析各维度趋势 */
    analyzeDimensionTrends(history) {
        const trends = {};
        const recent = history.slice(-3);
        for (const dim of recent[0]?.dimensionScores ?? []) {
            const scores = recent.map((h) => h.dimensionScores.find((d) => d.dimensionId === dim.dimensionId)?.rawScore ?? 0);
...
//# sourceMappingURL=simple-evolution.js.map