...
import { TaskAnalyzer, TeamComposer, TeamManager, globalWorkerRegistry, } from "../index.js";
// ============================================================
// TaskAnalyzer 测试
// ============================================================
...
describe("TeamComposer", () => {
    ...
    const TEST_RULES = [
        {
            id: "research-heavy",
            match: (f) => f.needsResearch && f.complexity === "high",
            team: [
        ...
        {
            id: "quick-output",
            match: (f) => f.length === "short" && !f.needsResearch,
            team: [
    ...
    it("应匹配视觉内容规则", () => {
        ...
        expect(workers.some((w) => w.role === "uiux-designer")).toBe(true);
    });
    it("应匹配快速出稿规则", () => {
        const features = {
            domain: "lifestyle",
    ...
    it("defsToWorkers 应生成正确的 IWorker 数组", () => {
        const defs = [{ role: "writer", priority: "essential" }];
