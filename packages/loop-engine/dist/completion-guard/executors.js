...
/** 创建通用证据记录 */
function makeEvidenceRecord(goalId, methodType, passed, evidence, durationMs) {
    return {
        goalId,
        method: methodType,
...
export class CommandExecutor {
    ...
    async execute(method, ctx) {
        ...
        try {
            ...
            return makeEvidenceRecord("", // goalId 由调用方填充
            "command", passed, {
                type: "command",
                command: cmd.command,
                exitCode,
            ...
            const expected = cmd.expectExitCode ?? 0;
            const passed = exitCode === expected;
            return makeEvidenceRecord("", "command", passed, {
                type: "command",
                command: cmd.command,
                exitCode,
...
export class TestExecutor {
    ...
    async execute(method, ctx) {
        ...
        try {
                        ...
                        for (const suite of result.testResults) {
                            ...
                            for (const assert of suite.assertionResults ?? []) {
                                if (assert.status === "passed")
                                    ...
                                    failedTests++;
                                    failedTestNames.push(assert.ancestorTitles?.concat(assert.name).join(" > ") ?? assert.name);
                                }
                            }
                        }
                    }
            ...
            const err = error;
            return makeEvidenceRecord("", "test", false, {
                type: "test",
                runner,
                totalTests: 0,
                passedTests: 0,
...
export class LintExecutor {
    ...
    async execute(method, ctx) {
        ...
        try {
            ...
            const err = error;
            return makeEvidenceRecord("", "lint", false, {
                type: "lint",
                tool: cfg.tool,
                errors: 1,
                warnings: 0,
...
export class BrowserExecutor {
    ...
    async execute(method, _ctx) {
        ...
        if (!pw) {
            ...
            return makeEvidenceRecord("", "browser_check", false, // 未通过（因为没有实际验证）
            {
                type: "browser_check",
                url: cfg.url,
        ...
        try {
            ...
            return makeEvidenceRecord("", "browser_check", allPassed && !hasConsoleErrors, {
                type: "browser_check",
                url: cfg.url,
                screenshot,
                assertions,
            ...
            const err = error;
            return makeEvidenceRecord("", "browser_check", false, {
                type: "browser_check",
                url: cfg.url,
                assertions: [],
...
// 5. FileExistenceExecutor — 文件存在性
// ============================================================
export class FileExistenceExecutor {
    methodType = "file_exists";
    async execute(method, ctx) {
        const cfg = method;
        const start = Date.now();
        try {
            ...
            return makeEvidenceRecord("", "file_exists", passed, {
                type: "file_exists",
            ...
            const err = error;
            return makeEvidenceRecord("", "file_exists", false, {
                type: "file_exists",
                matchedPaths: [],
            }, Date.now() - start);
        }
...
// 6. ContentMatchExecutor — 内容正则匹配
// ============================================================
export class ContentMatchExecutor {
    methodType = "content_match";
    async execute(method, ctx) {
        const cfg = method;
        const start = Date.now();
        ...
        try {
            ...
            const err = error;
            return makeEvidenceRecord("", "content_match", false, {
                type: "content_match",
                matchedLines: [],
 ...
 */
export class LLMAssertionExecutor {
    methodType = "llm_assertion";
    llmProvider;
    ...
    async execute(method, ctx) {
        ...
        const start = Date.now();
        if (!this.llmProvider) {
            return makeEvidenceRecord("", "llm_assertion", false, {
                type: "llm_assertion",
        ...
        try {
            ...
            return makeEvidenceRecord("", "llm_assertion", parsed.judgement === "pass" && (parsed.confidence ?? 0) >= 0.7, {
                type: "llm_assertion",
                model: "llm-provider",
