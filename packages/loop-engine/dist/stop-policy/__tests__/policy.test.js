...
import { evaluateStop, isSignificantImprovement, StopPolicy, DEFAULT_STOP_POLICY_CONFIG, } from "../policy.js";
const baseEval = (score, opts) => ({
    totalScore: score,
    weightedScore: score,
    passed: score >= 75,
    excellent: score >= 90,
...
describe("StopPolicy", () => {
    ...
    it("P0: all_goals_verified + quality not met → continue", () => {
        const guardResult = {
            ...
            stopCondition: {
                ...
                verifiedGoals: [{ goalId: "g1", evidence: {} }],
                totalIterations: 1,
                totalDurationMs: 0,
            },
            evidences: [],
        ...
        const decision = evaluateStop(baseCtx({ guardResult, evaluation: baseEval(70) }), { ...DEFAULT_STOP_POLICY_CONFIG, minQualityScore: 75 });
    ...
    it("P0: all_goals_verified + quality met → stop", () => {
        ...
        const decision = evaluateStop(baseCtx({ guardResult, evaluation: baseEval(80) }), { ...DEFAULT_STOP_POLICY_CONFIG, minQualityScore: 75 });
        expect(decision.stop).toBe(true);
    ...
    it("P2: 分数微降不超过阈值 → continue", () => {
        const decision = evaluateStop(baseCtx({ iteration: 2, evaluation: baseEval(72), lastScore: 75 }), DEFAULT_STOP_POLICY_CONFIG);
        expect(decision.stop).toBe(false);
    });
    it("P2: 分数下降超过阈值 → degradation stop", () => {
        const decision = evaluateStop(baseCtx({ iteration: 2, evaluation: baseEval(60), lastScore: 75 }), DEFAULT_STOP_POLICY_CONFIG);
        expect(decision.stop).toBe(true);
        expect(decision.reason).toBe("degradation");
    });
    it("P3: 停滞检测 → stagnation_pivot stop", () => {
        const decision = evaluateStop(baseCtx({ iteration: 3, stagnationCount: 2, evaluation: baseEval(70) }), DEFAULT_STOP_POLICY_CONFIG);
        expect(decision.stop).toBe(true);
        expect(decision.reason).toBe("stagnation_pivot");
    });
    it("P4: maxIterations → stop", () => {
        const decision = evaluateStop(baseCtx({ iteration: 5, evaluation: baseEval(60) }), { ...DEFAULT_STOP_POLICY_CONFIG, maxIterations: 5 });
        expect(decision.stop).toBe(true);
