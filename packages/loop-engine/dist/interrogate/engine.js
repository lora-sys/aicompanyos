import { INTERROGATE_SYSTEM_PROMPT, buildQuestionGenerationPrompt, buildSufficiencyCheckPrompt, buildFollowUpPrompt, } from "./prompts.js";
import { z } from "zod";
import { FallbackStrategy, createLLMParser, } from "../utils/llm-structured-output.js";
// 生成唯一 ID 的辅助函数
function generateId() {
        ...
        return items.map((item, index) => ({
            ...
            hints: Array.isArray(item.hints)
                ...
                : Array.isArray(item.options)
                    ? item.options.map((h) => String(h))
                    : Array.isArray(item.examples)
                        ? item.examples.map((h) => String(h))
                        : undefined,
...
function getEmojiForDimension(dimension) {
    const lower = dimension.toLowerCase();
    if (lower.includes("主题") || lower.includes("topic") || lower.includes("方向"))
        return "📌";
    if (lower.includes("读者") || lower.includes("目标") || lower.includes("audience"))
        return "👥";
    if (lower.includes("风格") || lower.includes("style"))
        return "🎨";
    if (lower.includes("格式") || lower.includes("format"))
        return "📋";
    if (lower.includes("长度") || lower.includes("字数"))
        return "📏";
    return "❓";
}
// 从多行文本中尝试解析为问题列表
function parseLinesAsQuestions(text) {
    const questions = [];
...
export class InterrogateEngine {
     ...
     */
    async startSession(taskId, userInput) {
    ...
    async shouldContinue(session) {
        ...
        const response = await this.llmProvider.chat([
            ...
            {
                role: "user",
                content: buildSufficiencyCheckPrompt(session.collectedContext, session.originalInput),
            },
        ]);
        try {
    ...
    async generateFollowUpQuestions(session) {
        ...
        const response = await this.llmProvider.chat([
            ...
            {
                role: "user",
                content: buildFollowUpPrompt(session.collectedContext, session.originalInput, previousAnswers),
            },
        ]);
        const newQuestions = parseQuestions(response).slice(0, this.maxQuestionsPerRound);
        // 如果没有新问题，直接标记完成
