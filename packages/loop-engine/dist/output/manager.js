...
export class ArtifactManager {
    ...
    inferType(filename) {
        if (filename.endsWith(".md") || filename.endsWith(".markdown")) {
            if (filename.includes("blog") || filename.includes("article"))
                return "blog";
            if (filename.includes("design") || filename.includes("spec"))
                return "design-doc";
        }
        if (filename.includes("tweet") || filename.includes("post"))
            return "tweet";
    ...
    async createHTMLArtifact(params) {
        ...
        const resolvedStyle = this.mergeStyleConfig(params.styleConfig ?? {});
        const htmlContent = this.renderMarkdownToHTML(params.markdownContent, params.title, params.metadata, resolvedStyle);
        // 同时保存原始 Markdown 为 .md 文件
        const mdName = params.name.replace(/\.html$/, '.md');
        await this.createArtifact({ name: mdName, content: params.markdownContent, type: this.inferType(mdName) });
        // 创建 HTML 文件
     ...
     */
    renderMarkdownToHTML(markdown, title, metadata, styleConfig = DEFAULT_STYLE_CONFIG) {
