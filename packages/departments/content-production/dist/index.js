        ...
        extraDimensions: [
            {
                ...
                weight: 5,
                maxScore: 20,
                scoringGuide: "20分: Emoji 8-12个，每个增强表达不显多余\n" +
...
export class ContentProductionDepartment {
    /** 部门标识 */
    static DEPARTMENT_ID = "content-production";
    /** 部门名称 */
    static DEPARTMENT_NAME = "内容产出部";
    /** 当前版本 */
    ...
    getConfig(contentType) {
        if (!ContentProductionDepartment.SUPPORTED_TYPES.includes(contentType)) {
            throw new Error(`[ContentProductionDepartment] 不支持的内容格式: "${contentType}"。` +
