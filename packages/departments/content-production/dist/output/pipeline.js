 ...
 */
async function executeFormatConverter(content, params) {
...
function markdownToHtml(md) {
    let html = md;
    // 代码块 → <pre><code>
    html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (_match, lang, code) => `<pre><code class="language-${lang || ''}">${escapeHtml(code.trim())}</code></pre>`);
    // 行内代码 → <code>
 ...
 */
async function executeMetadataInjector(content, params, context) {
    ...
    if (meta.date)
        metaParts.push(`日期: ${meta.date}`);
    if (meta.tags && Array.isArray(meta.tags)) {
        metaParts.push(`标签: ${meta.tags.join(", ")}`);
    }
 ...
 */
async function executePlatformAdapter(content, params, context) {
...
// ============================================================
/** 微信公众号 HTML 适配 */
function adaptForWechat(html, ctx) {
    const title = ctx.metadata?.title ?? "未命名文章";
    const author = ctx.metadata?.author ?? "";
    // 微信兼容的内联 CSS 样式
    const wechatStyle = `
<style>
...
/** 小红书卡片式 HTML 适配 */
function adaptForXiaohongshu(content, ctx) {
...
/** 抖音脚本纯文本格式化 */
function adaptForDouyin(content, _ctx) {
    ...
    const formatted = lines.map((line) => {
        ...
        if (/^\[画面\]/.test(line))
            return `  📷 ${line.replace(/^\[画面\]\s*/, "")}`;
        if (/^\[音效\]/.test(line))
            return `  🔊 ${line.replace(/^\[音效\]\s*/, "")}`;
        if (/^\[字幕\]/.test(line))
            return `  💬 ${line.replace(/^\[字幕\]\s*/, "")}`;
        if (/^\[口播\]|\[文案\]/.test(line))
            return `  🎙️ ${line.replace(/^\[(口播|文案)\]\s*/, "")}`;
        return line;
    }).join("\n");
    const header = [
        "┌─────────────────────────────────────────┐",
...
function adaptForSubstack(content, ctx) {
    const title = ctx.metadata?.title ?? "Newsletter";
    const author = ctx.metadata?.author ?? "";
 ...
 * 质量检查处理器 — 发布前最后安检
 */
async function executeQualityChecker(content, params, _context) {
    const failures = [];
    let passed = true;
    for (const check of params.checks) {
        switch (check.type) {
            case "word_count_min": {
                const min = check.params?.min ?? 100;
            ...
            case "word_count_max": {
                const max = check.params?.max ?? 10000;
                const count = content.length;
                if (count > max) {
            ...
            case "custom_regex": {
                const pattern = check.params?.pattern;
                if (pattern) {
...
export class OutputPipeline {
     ...
     */
    async process(rawContent, context) {
 ...
 */
export function createDefaultOutputPipeline(contentType, metadata) {
    switch (contentType) {
        case "article":
            return {
                primaryFormat: "html",
