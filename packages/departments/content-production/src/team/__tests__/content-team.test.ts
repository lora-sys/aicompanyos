...
import type { TaskFeatures, WorkerRole, TeamContext } from "@aicos/loop-engine";

// ============================================================
// Mock: 简化的 TeamContext（不需要完整实现）
// ============================================================

const BASE_CONTEXT: TeamContext = {
  departmentId: "content-production",
  availableRoles: ["writer", "critic", "researcher", "uiux-designer", "reviewer"] as WorkerRole[],
};
...
describe("ContentTeamRules — 8 条规则匹配", () => {
  it("R1: 高复杂度+调研+视觉+premium → 全明星5人", async () => {
    ...
    const team = await mgr.composeTeam(
      "写一篇关于AI Agent架构的深度技术文章，需要调研最新论文并配图",
      BASE_CONTEXT
    );
...
describe("ContentTeamRules — 规则集完整性", () => {
  ...
  it("必须包含兜底规则", () => {
    ...
    expect(fallback!.match({} as TaskFeatures)).toBe(true);
  });
