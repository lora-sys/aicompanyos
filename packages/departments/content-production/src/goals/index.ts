 ...
 */
function contentMatch(pattern: RegExp | string, target = "**/*.md"): AcceptanceGoal["verifyBy"][0] {
 ...
 */
function fileExists(path: string, minSizeBytes?: number): AcceptanceGoal["verifyBy"][0] {
...
export const ARTICLE_GOAL_TEMPLATES: DepartmentGoalTemplate[] = [
  {
    match: {
      contentType: "article",
      keywords: ["文章", "写", "写作", "公众号", "知乎", "blog"],
    },
    generate: (stepId: string): AcceptanceGoal[] => [
      {
...
export const SEED_GOAL_TEMPLATES: DepartmentGoalTemplate[] = [
  {
    match: {
      contentType: "seed",
...
export const SHORT_VIDEO_GOAL_TEMPLATES: DepartmentGoalTemplate[] = [
  {
    match: {
...
export const NEWSLETTER_GOAL_TEMPLATES: DepartmentGoalTemplate[] = [
  {
    match: {
...
// ============================================================
// 工厂函数：根据 ContentType 获取所有部门目标模板
// ============================================================

import type { ContentType } from "@aicos/loop-engine";

/** 所有部门目标模板的汇总映射 */
const ALL_DEPARTMENT_GOALS: Record<ContentType, DepartmentGoalTemplate[]> = {
 ...
 */
export function getDepartmentGoalTemplates(contentType: ContentType): DepartmentGoalTemplate[] {
