...
export class ResearcherAgent {
    tools;
    llmProvider;
    static AGENT_TYPE = "researcher";
    static SYSTEM_PROMPT = `你是一个专业的技术资料研究员。你的任务是根据写作主题搜索和整理高质量的参考资料。
...
输出格式：返回 JSON，包含 sources 数组和 summary 文本。`;
     ...
     * 通过 MCP Exa 搜索资料
     */
    async searchViaMCP(topic, taskId) {
     ...
     */
    async searchViaWebSearch(topic, taskId) {
    ...
    parseMCPExaResults(data) {
        ...
        return data.map((item) => ({
            ...
            keyPoints: Array.isArray(item.keyPoints)
                ? item.keyPoints.slice(0, 3)
                : [],
            credibility: "high", // Exa 搜索结果通常质量较高
        })).filter((s) => s.title && s.url);
    }
    /**
     * 调用 LLM 整理所有来源为结构化摘要
     */
    async summarizeSources(input, sources) {
        if (sources.length === 0) {
