...
- 即使引用第三方库的官方文档是 Python 示例，也应将其转写为 TypeScript 等价实现`;
  ...
  async execute(params: {
    ...
    // 检测用户在拷问阶段指定的编程语言偏好
    const userLangPreference = this.detectLanguagePreference(
      params.context.interrogationResults
    );

    // === Loop Engineering: 从 extensions 中提取 Critic 反馈 ===
    const criticFeedback = params.context.extensions?.criticFeedback as string | undefined;
    const rewriteRound = params.context.extensions?.rewriteRound as number | undefined;
    ...
    const input: WriterInput = {
      ...
      context: {
        ...
        uiuxGuidance: params.context.uiuxGuidance as
          | UIUXSkillOutput
          | undefined,
  ...
  private extractLengthConstraint(
    selfExperience?: { lessons?: string[]; content?: string }
  ): string | undefined {
    if (!selfExperience) return undefined;
    ...
    const lengthPatterns = sources.find((s) =>
      /(\d+)\s*[-~至]\s*(\d+)\s*(字节|字符|字|bytes?|chars?)|(篇幅|字数|长度).{0,20}(控制|限制|不超过|以内)/i.test(s)
    );
  ...
  private detectLanguagePreference(
    ...
    if (!interrogationResults) return undefined;
    ...
    for (const dim of langDimensions) {
      ...
      if (value) {
        ...
        const match = value.match(
          /(TypeScript|JavaScript|Python|Java|Go|Rust|C\+\+|Kotlin|Swift)/i
        );
  ...
  // 内部：完整的写作工作流
  private async writingWorkflow(input: WriterInput): Promise<WriterOutput> {
    const usedTools: string[] = [];

    // 步骤1：获取 UI/UX 设计指导
    let uiGuidance: object;
    try {
  ...
  // 步骤1：获取 UI/UX 设计指导
  private async getUIGuidance(input: WriterInput): Promise<object> {
  ...
  // 后处理：硬性长度裁剪（LLM 可能忽略篇幅约束）
  private enforceLengthLimit(content: string, maxChars = 15000): string {
    if (content.length <= maxChars) return content;

    console.warn(
      `[WriterAgent] 内容超限: ${content.length} chars > ${maxChars} chars，执行智能裁剪`
    );
    ...
    for (const section of sections) {
      if ((trimmed + section).length > maxChars) break;
      trimmed += (trimmed ? "\n" : "") + section;
    }
   ...
   */
  private checkTopicDrift(content: string, input: WriterInput): string {
    // 从任务描述中提取核心主题词
    ...
    if (topicKeywords.length === 0) return content; // 无法提取关键词，跳过检测
    ...
    if (matchRatio < 0.5) {
      console.warn(
        `[WriterAgent] Topic Drift 检测: 只匹配 ${matchedKeywords.length}/${topicKeywords.length} 个关键词 (${Math.round(matchRatio * 100)}%)`
      );
      console.warn(
        `[WriterAgent]   期望主题关键词: [${topicKeywords.join(", ")}]`
      );
      console.warn(
        `[WriterAgent]   实际匹配关键词: [${matchedKeywords.join(", ") || "无"}]`
      );
