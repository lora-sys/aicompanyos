...
import type { CriticInput, CriticOutput } from "./types.js";
...
export class CriticAgent implements AgentExecutor, IEvaluatorAgent<WriterOutput> {
...
即使通过也要给出改进空间。`;
  ...
  private customDimensions: Array<{
    id: string;
    name: string;
    description: string;
    maxScore: number;
  ...
  setCustomDimensions(dimensions: Array<{
    ...
    console.log(
      `[CriticAgent] 已设置 ${dimensions.length} 个部门专属评估维度: ` +
  ...
  async execute(params: {
    ...
    const writerOutput =
      params.previousOutputs[params.step.stepId] ??
      Object.values(params.previousOutputs).find(
        (o) => o && typeof o.content === "string" && o.content.length > 0,
      );

    if (!writerOutput) {
      throw new Error(
        `未找到 Writer 的产出。可用的 keys: [${Object.keys(params.previousOutputs).join(", ")}], 当前 stepId: ${params.step.stepId}`,
      );
    }

    const input: CriticInput = {
      taskId: params.context.taskId,
      writerOutput: { content: writerOutput.content } as CriticInput["writerOutput"],
  ...
  async evaluate(
    ...
    // a. 提取内容
    const content = output.content ?? (output as unknown as string);
   ...
   */
  private parseGradingResponse(response: string, criteria: GradingCriteria): GradingResult {
    const zeroFallback = this.getZeroGradingResult("schema 解析回退", criteria);

    try {
      const parser = createLLMParser({
        schema: gradingResultSchema as any,
      ...
      // Schema 解析失败 → 回退到正则提取
      console.warn("[CriticAgent.evaluate] Schema 解析失败，尝试正则回退:", (result as any).error);
   ...
   */
  private mapToGradingResult(raw: any, criteria: GradingCriteria): GradingResult {
    ...
    // 映射 suggestions
    const suggestions = (raw.suggestions ?? []).map((s: any) => ({
      dimensionId: s.dimensionId ?? "general",
      severity: (s.severity ?? "major") as GradingResult["suggestions"][number]["severity"],
   ...
   */
  private buildDimensionMapping(criteria: GradingCriteria): Map<string, string> {
  ...
  private matchDimensionKey(
    llmDimensions: Record<string, any>,
    targetId: string,
    targetName: string,
  ): string {
    // 1. 精确匹配 ID
    if (targetId in llmDimensions) return targetId;
  ...
  private fallbackParseGrading(response: string, criteria: GradingCriteria): GradingResult {
    try {
  ...
  private getZeroGradingResult(reason: string, criteria: GradingCriteria): GradingResult {
  ...
  // 内部：审核流程
  private async review(input: CriticInput): Promise<CriticOutput> {
    // 步骤1：读取 Writer 产出的内容
    const content =
      "content" in input.writerOutput
  ...
  // 构造审核提示词
  private buildReviewPrompt(input: CriticInput, content: string): string {
    let prompt = `## 原始任务\n${input.originalTask}\n\n`;
    ...
    // 添加拷问要求
    if (
      input.context.interrogationResults &&
  ...
  private parseReviewResponse(response: string): CriticOutput {
    ...
    try {
      ...
      const parser = createLLMParser({
        schema: criticOutputSchema as any,
        fallback: zeroFallback,
        strategy: FallbackStrategy.RETURN_FALLBACK,
      ...
      if (result.success && result.data) {
        ...
        return {
          ...
          dimensions: (d.dimensions ?? {}) as CriticOutput["dimensions"],
          passed: d.passed ?? (d.overallScore ?? 0) >= this.PASS_THRESHOLD,
          suggestions: (d.suggestions ?? []).map((s: any) => ({
            type: (s.type ?? "content") as CriticOutput["suggestions"][number]["type"],
            severity: (s.severity ?? "major") as CriticOutput["suggestions"][number]["severity"],
            location: s.location,
            description: s.description ?? "",
            suggestion: s.suggestion ?? "",
          })),
      ...
      // Schema 解析失败 → 回退到正则提取
      console.warn("[CriticAgent] Schema 解析失败，尝试正则回退:", (result as any).error);
  ...
  /** 正则回退：当 Zod schema 不匹配时 */
  private fallbackParseReview(response: string): CriticOutput {
    try {
      const jsonMatch = response.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error("未找到 JSON");

      const parsed = JSON.parse(jsonMatch[0]) as Partial<CriticOutput>;
  ...
  /** 安全的全零分数响应 */
  private getZeroScoreResponse(reason: string): CriticOutput {
    return {
      overallScore: 0,
      dimensions: {
