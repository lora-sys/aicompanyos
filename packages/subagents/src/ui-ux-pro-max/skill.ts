...
import { LLMStructuredOutput, createLLMParser, FallbackStrategy } from "@aicos/loop-engine/utils";
...
// 默认值（当 Zod 解析失败时使用）
const DEFAULT_UIUX_OUTPUT: UIUXSkillOutput = {
  colorPalette: {
    primary: "#1976D2",
    secondary: "#424242",
    accent: "#FF4081",
...
// 错误回退值（置信度更低，标记为解析失败）
const FALLBACK_UIUX_OUTPUT: UIUXSkillOutput = {
  ...DEFAULT_UIUX_OUTPUT,
...
export class UIUXProMaxSkill {
  ...
  static readonly SKILL_DESCRIPTION =
    "UI/UX 设计指导专家，提供色彩方案、排版规则、布局建议和设计令牌";

  private llm: LLMProvider;
  // 使用 LLMStructuredOutput 替代手写解析
  private parser: LLMStructuredOutput<UIUXSkillOutput>;

  constructor(llm: LLMProvider) {
    this.llm = llm;
    // 初始化 Zod 驱动的解析器
    this.parser = createLLMParser<UIUXSkillOutput>({
      schema: uiUXOutputSchema as any,
  ...
  async execute(input: UIUXSkillInput): Promise<UIUXSkillOutput> {
    // 检查是否有现有 design.mdx
  ...
  private async generateDesignGuidance(
    input: UIUXSkillInput
  ): Promise<UIUXSkillOutput> {
...
请基于现有设计规范进行增量优化。如果新任务与现有风格一致，保持并微调；如果有明显差异，给出调整建议并说明原因。`;
  ...
  // 构建用户提示词
  private buildPrompt(input: UIUXSkillInput): string {
   ...
   */
  private parseResponse(response: string): UIUXSkillOutput {
