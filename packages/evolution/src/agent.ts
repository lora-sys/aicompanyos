...
- self.md: 系统自我认知 / 经验积累`;
  ...
  private patternExtractor: IPatternExtractor;
  private diffGenerator: IDiffGenerator;
  private autoMerger: IAutoMerger;
  ...
  async run(params: {
    ...
    // 4. 执行对应模式的进化流程
    const evolveParams: EvolutionParams = {
      evidenceChain,
      evolutionDocs,
      taskId,
      taskSuccess,
  ...
  private async regularEvolve(params: EvolutionParams): Promise<EvolutionResult> {
    ...
    const selfDiff = this.diffGenerator.generateSelfDiff(
      currentSelf ?? { experiences: [], totalTasksCompleted: 0, totalSuccessRate: 0, capabilities: [], limitations: [], lastUpdated: "", knownCapabilities: [], knownLimitations: [] },
      safePatterns,
      taskSuccess,
  ...
  // 推断任务类型
  private inferTaskType(evidenceChain: IEvidenceReader): string {
    const decisions = evidenceChain.getEntriesByType("decision");
    if (decisions.length === 0) return "unknown";

    // 从决策中推断主要任务类型
    const agentTypes = new Set(decisions.map((d) => (d as any).agentType));
    if (agentTypes.has("ui-ux")) return "ui-ux-design";
    if (agentTypes.has("writer")) return "content-creation";
    if (agentTypes.has("critic")) return "review";
    return "general";
  }

  // 深度反思（仅深度进化时调用）
  private async deepReflect(evidenceChain: IEvidenceReader, taskSuccess: boolean): Promise<string | null> {
    ...
    const response = await this.llmProvider.chat([
      {
        role: "system",
        content:
          `基于以下完整的任务执行证据链，进行深度反思。任务${taskSuccess ? "成功" : "失败"}。\n` +
