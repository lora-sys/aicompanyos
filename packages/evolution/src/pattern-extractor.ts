...
export class PatternExtractor {
  ...
  async extractPatterns(evidenceChain: IEvidenceReader): Promise<ExtractedPatterns> {
    // 获取各类记录
    const decisions = evidenceChain.getEntriesByType("decision") as DecisionTraceEntry[];
    const toolCalls = evidenceChain.getEntriesByType("tool_call") as ToolCallTraceEntry[];
  ...
  private async batchAnalyze(
    ...
    const uxEntries = allEntries.filter(
      (e) => e.type === "decision" && (e as DecisionTraceEntry).agentType === "ui-ux",
    );
    const uxSummary = uxEntries.length > 0
      ? `## UI/UX 设计决策\n${uxEntries.map((e) => { const d = e as DecisionTraceEntry; return `${d.decisionPoint}: ${d.finalChoice}${d.outputReasoning ? `\n理由: ${d.outputReasoning}` : ""}`; }).join("\n\n")}`
      : "## UI/UX 设计决策\n（无）";
    ...
    let preferences: PreferencePatterns = {};
    let toolUsage: ToolUsagePatterns = { frequentTools: [], failedTools: [], usageEfficiencyTips: [] };
    let uxDecisions: UXDecisionPatterns = {};

    try {
      const parsed = JSON.parse(response) as Record<string, unknown>;
      if (parsed.preferences && typeof parsed.preferences === "object") {
        preferences = parsed.preferences as PreferencePatterns;
      }
      if (parsed.toolUsage && typeof parsed.toolUsage === "object") {
        toolUsage = parsed.toolUsage as ToolUsagePatterns;
      }
      if (parsed.uxDecisions && typeof parsed.uxDecisions === "object") {
        uxDecisions = parsed.uxDecisions as UXDecisionPatterns;
      }
    } catch {
      // JSON 解析失败，降级到规则引擎
      return this.ruleBasedExtract(decisions, toolCalls, allEntries);
  ...
  // 偏好分析的规则引擎兜底方案
  private fallbackPreferenceAnalysis(decisions: DecisionTraceEntry[]): PreferencePatterns {
    if (decisions.length === 0) return {};

    const newPreferences: Array<{ key: string; value: string; confidence: number }> = [];
  ...
  // UX 分析的规则引擎兜底方案
  private fallbackUXAnalysis(entries: TraceEntry[]): UXDecisionPatterns {
    const uxEntries = entries.filter(
      (e) => e.type === "decision" && (e as DecisionTraceEntry).agentType === "ui-ux",
    );

    if (uxEntries.length === 0) return {};

    const layoutPreferences: string[] = [];
    const colorKeywords = new Set<string>();

    for (const e of uxEntries) {
      const d = e as DecisionTraceEntry;
      const text = `${d.decisionPoint} ${d.finalChoice}`.toLowerCase();
    ...
    const result: UXDecisionPatterns = {};
    if (layoutPreferences.length > 0) result.layoutPreferences = layoutPreferences;
    if (colorKeywords.size > 0) result.colorChanges = { palette: Object.fromEntries(Array.from(colorKeywords).map((c) => [c, "detected"])), reasoning: "从决策记录中提取" };

    return result;
  }

  // 带 30s 超时保护的 LLM 调用
  private async callWithTimeout(messages: Array<{ role: "system" | "user"; content: string }>): Promise<string> {
  ...
  // 提取成功模式（基于证据链中的正向指标）
  private extractSuccessPatterns(entries: TraceEntry[]): string[] {
    const patterns: string[] = [];

    // 从成功的工具调用中提取模式
    const successfulCalls = entries.filter(
      (e) => e.type === "tool_call" && (e as ToolCallTraceEntry).success,
    );
    ...
    // 从高置信度决策提取
    const highConfidenceDecisions = entries.filter(
      (e) => e.type === "decision" && ((e as DecisionTraceEntry).confidence ?? 0) > 0.85,
    );
  ...
  // 提取失败模式（基于证据链中的负向指标）
  private extractFailurePatterns(entries: TraceEntry[]): string[] {
    const patterns: string[] = [];

    // 失败的工具调用
    const failedCalls = entries.filter(
      (e) => e.type === "tool_call" && !(e as ToolCallTraceEntry).success,
    );
    for (const fc of failedCalls) {
      const tc = fc as ToolCallTraceEntry;
      patterns.push(`工具 ${tc.toolName} 失败: ${tc.errorMessage || "未知错误"}`);
    }

    // 低置信度决策
    const lowConfidenceDecisions = entries.filter(
      (e) => e.type === "decision" && ((e as DecisionTraceEntry).confidence ?? 1) < 0.5,
    );
  ...
  // 工具分析的统计兜底方案
  private fallbackToolAnalysis(toolCalls: ToolCallTraceEntry[]): ToolUsagePatterns {
    const toolStats = new Map<string, { count: number; totalDuration: number; fails: number; errors: Set<string> }>();
    ...
    for (const tc of toolCalls) {
      ...
      if (!tc.success) {
        existing.fails++;
        if (tc.errorMessage) existing.errors.add(tc.errorMessage);
      }
