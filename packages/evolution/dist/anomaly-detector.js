...
export class AnomalyDetector {
    ...
    // 检测共识失败率
    detectConsensusFailure(recentTasks) {
    ...
    // 检测 Replan 频率
    detectReplanFrequency(recentTasks) {
        // 检查是否有任务的 replanCount 超过阈值
        const maxReplan = Math.max(...recentTasks.map((t) => t.metrics.replanCount), 0);
        const avgReplan = recentTasks.reduce((sum, t) => sum + t.metrics.replanCount, 0) / recentTasks.length;
        // 使用最大值和平均值的综合判断
    ...
    // 检测用户手动修改
    detectUserModification(recentTasks) {
        // 统计用户修改次数
        const tasksWithMods = recentTasks.filter((t) => t.metrics.userModifications != null && t.metrics.userModifications > 0);
        const totalMods = tasksWithMods.reduce((sum, t) => sum + (t.metrics.userModifications ?? 0), 0);
