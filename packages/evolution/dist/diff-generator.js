export class DiffGenerator {
    ...
    generateSelfDiff(_currentSelf, patterns, taskSuccess, taskType, criticSummary, guardSummary) {
        ...
        if (relevantPatterns.length > 0) {
            ...
            if (patterns.toolUsage?.frequentTools?.length) {
                altParts.push(`常用工具: ${patterns.toolUsage.frequentTools.map((t) => t.toolName).join(",")}`);
            }
            if (patterns.preferences?.newPreferences?.length) {
                altParts.push(`新偏好: ${patterns.preferences.newPreferences.map((p) => p.key).join(",")}`);
            }
    ...
    // 根据模式生成经验教训
    generateLesson(patterns, taskSuccess, taskType, criticSummary, guardSummary) {
        const lessons = [];
        ...
        if (lessons.length === 0) {
            ...
            if (patterns.toolUsage?.frequentTools?.length) {
                parts.push(`主要使用工具: ${patterns.toolUsage.frequentTools.slice(0, 3).map((t) => t.toolName).join(", ")}`);
            }
            if (patterns.toolUsage?.failedTools?.length) {
                parts.push(`工具失败: ${patterns.toolUsage.failedTools.map((t) => t.toolName).join(", ")}`);
            }
            if (patterns.preferences?.newPreferences?.length) {
                parts.push(`发现新偏好: ${patterns.preferences.newPreferences.slice(0, 3).map((p) => p.key).join(", ")}`);
            }
